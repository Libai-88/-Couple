; installer.nsh - NSIS custom hooks for Couple installer
;
; Owns the Windows overlay boundary for Couple installs. The installer may
; replace Couple-owned program files, while user/runtime state stays outside
; $INSTDIR.
...
!macro hanakoFindRunningProcesses _RETURN
  !insertmacro hanakoFindProcess Couple.exe ${_RETURN}
  ${If} ${_RETURN} != 0
...
!macro hanakoKillRunningProcesses _FORCE
  !insertmacro hanakoKillProcess Couple.exe ${_FORCE}
...
!macro hanakoVerifyInstallSurface
  ...
  Push $R2
  StrCpy $R2 ""
  !insertmacro hanakoRequireInstallSurfaceFile "$INSTDIR\${APP_EXECUTABLE_FILENAME}" "Couple.exe"
  ...
  ${If} $R2 != ""
    DetailPrint "Couple install surface self-check failed."
    FileOpen $0 "$INSTDIR\hanaagent-install-diagnostics.log" w
    FileWrite $0 "Couple install surface self-check failed.$\r$\n"
    FileWrite $0 "Install dir: $INSTDIR$\r$\n"
    FileWrite $0 "Missing or unreadable files:$R2$\r$\n"
    FileClose $0
    MessageBox MB_OK|MB_ICONSTOP "Couple installation is incomplete. Missing or unreadable files:$R2$\r$\n$\r$\nDiagnostic file:$\r$\n$INSTDIR\hanaagent-install-diagnostics.log"
    SetErrorLevel 1
  ...
  ${Else}
    ...
    DetailPrint "Couple install surface self-check passed."
  ${EndIf}
  Pop $R2
  Pop $0
...
!macro hanakoWriteInstallDirProcessFinder _SCRIPT
  ...
  FileWrite $0 `  Write-Output ("Couple-owned process still running: {0} pid={1} path={2}" -f $$_.Name, $$_.ProcessId, $$_.ExecutablePath)$\r$\n`
  FileWrite $0 `}$\r$\n`
...
!macro hanakoBypassOldUninstallerForUpdate
  ${If} ${isUpdated}
    DetailPrint "Update mode detected; bypassing the previous uninstaller and preparing a Couple-owned overlay."
...
!macro customCheckAppRunning
  ...
  ${If} $R0 == 0
    DetailPrint "Detected Couple-owned process in install directory; closing it before install."
    Sleep 500
    !insertmacro hanakoStopInstallDirProcesses
    ...
    hanako_check_install_dir_processes:
      ...
      ${If} $R0 == 0
        IntOp $R1 $R1 + 1
        DetailPrint "Waiting for Couple-owned install-directory processes to close."
        ${If} $R1 > 2
          DetailPrint "Couple-owned install-directory processes still running; asking user to retry."
  ...
  ${If} $R0 == 0
    DetailPrint "Detected Couple.exe, Hanako.exe, or hana-server.exe; closing them before install."
    ...
    hanako_check_processes:
      ...
      ${If} $R0 == 0
        IntOp $R1 $R1 + 1
        DetailPrint "Waiting for Couple.exe, Hanako.exe, or hana-server.exe to close."
        ${If} $R1 > 2
          DetailPrint "Couple.exe, Hanako.exe, or hana-server.exe still running; asking user to retry."
...
!macro hanakoRemoveOwnedInstallTrees
  ...
  DetailPrint "Removing Couple-owned install files"
  SetOutPath "$TEMP"
  RMDir /r "$INSTDIR\resources\server"
...
!macro customUnInstallCheck
  ...
  ${If} ${Errors}
    DetailPrint `Previous uninstaller could not be launched; preparing a Couple-owned overlay.`
  ${ElseIf} $R0 != 0
    DetailPrint `Previous uninstaller exited with code $R0; preparing a Couple-owned overlay.`
  ${EndIf}
  !insertmacro hanakoPrepareOwnedOverlay
  ClearErrors
...
!macro customUnInstallCheckCurrentUser
  ...
  ${If} ${Errors}
    DetailPrint `Previous current-user uninstaller could not be launched; continuing with Couple-owned overlay.`
  ${ElseIf} $R0 != 0
    DetailPrint `Previous current-user uninstaller exited with code $R0; continuing with Couple-owned overlay.`
  ${EndIf}
  !insertmacro hanakoPrepareOwnedOverlay
  ClearErrors
