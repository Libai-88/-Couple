...
export class Agent {
  ...
  buildSystemPrompt( options: BuildSystemPromptOptions = {}) {
    ...
    const parts = [
      isZh
        ? "你运行在 Couple 平台上，基于 OpenHanako 开源项目构建。"
        : "You are running on the Couple platform, built on the OpenHanako open-source project.",
    ];
    ...
    parts.push(isZh
      ? "\n## Session 文件与交付\n\n" +
        ...
        "当用户要求安装 skill package 时，使用 install_skill。GitHub 仓库用 github_url；当前 Couple server 可见的本机路径用 local_path 或 source={ type: 'path', path }；已经上传或登记为 SessionFile 的 .zip/.skill 包用 fileId 或 source={ type: 'session_file', fileId }。不要把手机/PWA 客户端路径当成 server 路径。\n\n" +
      ...
      : "\n## Session Files and Delivery\n\n" +
        ...
        "When the user asks you to install a skill package, use install_skill. Use github_url for GitHub repos; use local_path or source={ type: 'path', path } for paths visible to the current Couple server; use fileId or source={ type: 'session_file', fileId } for uploaded or registered .zip/.skill packages. Do not treat a phone/PWA client path as a server path.\n\n" +
    ...
    parts.push(isZh
      ? "\n## 可见 UI 上下文\n\n" +
        "当用户用「这个、当前、打开的、可见的、选中的、置顶的」等说法指代 Couple 界面里正在看的文件、预览或文件夹时，先调用 current_status 获取 ui_context，再决定要读哪个文件或目录。\n\n" +
        "ui_context 是用户当前可见界面的被动元信息，可能包含当前查看的文件夹、激活文件或预览标题、以及置顶 viewer 文件。它只描述 Couple 已收集到的 UI 视野；如果返回为空或不足以确定对象，向用户确认，不要猜路径。"
      : "\n## Visible UI Context\n\n" +
        "When the user refers to something in the Couple UI with words like current, open, visible, selected, pinned, this file, this folder, or what I am looking at, call current_status with the ui_context key before deciding which file or folder to inspect.\n\n" +
        "ui_context is passive metadata about the user's visible UI state. It may include the currently viewed folder, active file or preview title, and pinned viewer files. It only describes UI state Couple has collected; if it is empty or not enough to identify the target, ask the user instead of guessing a path."
    );
	    ...
	    if (this._isComputerUseAvailableForThisAgent()) {
	      parts.push(isZh
	        ? "\n## 本机应用控制\n\n" +
	          ...
	          "不要用 exec_command、AppleScript、osascript、open -a 或平台脚本控制 GUI 应用；这些路径会绕过 Couple 的应用审批列表，也更容易撞到系统隐私权限。" +
	        ...
	        : "\n## Desktop App Control\n\n" +
	          ...
	          "Do not use exec_command, AppleScript, osascript, open -a, or platform scripts to control GUI applications; those paths bypass Couple's app approval list and are more likely to hit OS privacy permissions. " +
