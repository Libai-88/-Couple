...
function createOpenClawIssue({ marker, id, name, version }) {
  return {
    ...
    name,
    version,
    message: `This package looks like an OpenClaw plugin (${marker}). Couple cannot install OpenClaw plugin zips directly. Use a Couple plugin package with manifest.json, or port the plugin to Couple's plugin SDK.`,
  };
}
