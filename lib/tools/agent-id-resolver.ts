 ...
 */
export function resolveAgentParam(agents: Array<{id: string; name?: string}>, raw: string | undefined) {
  if (!raw) return { ok: true, agentId: undefined };

  const byId = agents.find((a: {id: string; name?: string}) => a.id === raw);
  if (byId) return { ok: true, agentId: byId.id };

  const byName = agents.filter((a: {id: string; name?: string}) => a.name === raw);
