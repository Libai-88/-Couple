 ...
 *   B. local_path / source(path) — 从当前 Couple server 可见路径安装完整 package
...
export function createInstallSkillTool({ getUserSkillsDir, getConfig, resolveUtilityConfig, onInstalled, registerSessionFile, resolveSessionFile }: any) {
  ...
  return {
    name: "install_skill",
    label: "Install Skill",
    description: "Install a complete skill package into the shared skill pool, enabled only for the current Agent by default. Provide github_url for a GitHub repo, local_path for a package path visible to the current Couple server, fileId for an uploaded SessionFile package, or source as a typed FileRef such as { type: 'path', path } / { type: 'session_file', fileId }. The full package directory is installed so references/scripts/assets are preserved. Do not provide raw skill_content or a single SKILL.md file. If the safety review returns requiresRiskConfirmation, explain the risk to the user and call again with risk_accepted=true plus the returned risk_confirmation_token only after explicit user confirmation.",
    parameters: Type.Object({
      ...
      local_path: Type.Optional(
        Type.String({ description: "Skill package path visible to the current Couple server. Can point to a folder containing SKILL.md, .zip, or .skill. Relative paths resolve from the current session cwd." })
      ),
