/**
 * card-guide-tool.ts — Couple Interactive Card 设计手册工具
 *
...
// 视觉语言完全遵循 Couple 主题 + KAMI 墨染色系。

const HANDBOOK = `# Couple Interactive Card Design Handbook

You are generating HTML/SVG fragments that render inside an iframe in Couple's chat.
...
### KAMI extended palette (from Couple's notebook design language)
These are not injected as CSS variables but belong to Couple's full color spectrum. Use them directly as hex values.
...
### Chart palette (6 colors, all from Couple/KAMI ink-dyed family)
...
cursor: pointer;
\`\`\`
Hover: \`background: var(--accent-light)\`. No :active scale animation (Couple quiet motion principle).
...
| emoji | Couple uses SVG stroke icons only |
...
| Font-weight 700 | Couple allows 400/500 only. 600 for metrics only |
| Title Case / ALL CAPS | Couple uses sentence case exclusively |
...
- Default corner radius: \`rx="2"\` (Couple's square-corner style). Larger values only for deliberate pill shapes.
...
export function createCardGuideTool() {
  return {
    ...
    description:
      "Returns the Couple Interactive Card Design Handbook — CSS variables, colors, typography, layout rules, " +
