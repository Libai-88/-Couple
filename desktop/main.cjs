/**
 * Couple Desktop — Electron 主进程
 *
 * 职责：
 * 1. 创建启动窗口（splash）
 * 2. spawn() 启动 Couple Server
...
const APP_USER_MODEL_ID = "com.couple.app"; // Keep in sync with package.json build.appId.
...
// onboarding/主窗口白屏且无前端报错。此处硬崩，拒绝以不可用状态启动。
{
  ...
  if (!fs.existsSync(preloadPath)) {
    ...
    try { dialog.showErrorBox("Couple failed to start", msg); } catch {}
...
// 生产: ~/Library/Application Support/Hanako（历史目录，随 Couple 显示名保留）
// 开发: ~/Library/Application Support/Hanako-dev
const defaultHome = path.join(os.homedir(), ".couple");
  ...
  serverProcess.on("exit", async (code, signal) => {
    ...
    if (_serverRestartAttempts < 1) {
      ...
      try {
        ...
        dialog.showErrorBox("Couple Server", mt("dialog.serverRestartFailed", {
      ...
      dialog.showErrorBox("Couple Server", mt("dialog.serverMultipleCrash", {
        version: app?.getVersion?.() || "unknown",
        reason,
      }));
    }
...
function createTray() {
  ...
  tray.setToolTip(isDev ? "Couple (dev)" : "Couple");

  const buildMenu = () => Menu.buildFromTemplate([
    { label: mt("tray.show", null, "Show Couple"), click: () => showPrimaryWindow() },
...
function writeCrashLog(errorMessage) {
  ...
  const content = redactMainLogText([
    `=== Couple Crash Log ===`,
    `Couple: v${app?.getVersion?.() || "unknown"}`,
    `Time: ${timestamp}`,
    `Error: ${errorMessage}`,
...
<body>
  ...
  <footer class="watermark">
    ...
    <span class="watermark-text">Couple</span>
  </footer>
</body>
</html>`;
}
...
app.whenReady().then(async () => {
  try {
    ...
    console.log("[desktop] 启动 Couple Server...");
    await startServer();
    if (process.platform === "win32") {
      markGpuStartupPhase({
        hanakoHome,
    ...
    dialog.showErrorBox(
      mt("dialog.launchFailedTitle", null, "Couple Launch Failed"),
