...
try {
  ...
  showBootstrapError(
    "Couple Launch Failed",
    `Couple failed before HANA_HOME could be resolved.\n\n${err?.message || err}\n\nDiagnostic file:\n${diagnosticPath || diagnosticsDir}`,
  );
...
function verifyWindowsInstallSurfaceBeforeMain() {
  ...
  showBootstrapError("Couple Launch Failed", detail);
  exitAfterBootstrapFailure();
  return false;
}
...
function loadDesktopMain() {
  try {
    ...
    showBootstrapError(
      "Couple Launch Failed",
      `Couple failed before the desktop main process finished loading.\n\n${err?.message || err}\n\nDiagnostic file:\n${diagnosticPath || diagnosticsDir}`,
    );
    exitAfterBootstrapFailure();
  }
}
